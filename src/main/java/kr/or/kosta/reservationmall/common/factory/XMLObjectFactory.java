package kr.or.kosta.reservationmall.common.factory;
...
public class XMLObjectFactory {
	...
	Logger logger = Logger.getLogger(XMLObjectFactory.class);
	...
	public XMLObjectFactory(String objectMapperLocation) throws Exception {
		...
		try {
			...
			logger.debug("--- Object 생성 목록(" + beanList.getLength() + ") ---");
			for (int i = 0; i < beanList.getLength(); i++) {
				...
				logger.debug("[debug] : [" + beanType + "] " + beanName + " = " + beanClassName);

				if (beanType.equals("component") && beanName.equals("sqlSessionFactory")) {
					NodeList propertyList = bean.getChildNodes();
					if (propertyList.getLength() > 0) {
						SqlSessionFactoryCreator sqlSessionFactoryCreator = (SqlSessionFactoryCreator) objectList
								.get(beanName);
						for (int j = 0; j < propertyList.getLength(); j++) {
							...
							if (node.getNodeType() == Node.ELEMENT_NODE) {
								...
								SqlSessionFactory sqlSessionFactory = sqlSessionFactoryCreator
